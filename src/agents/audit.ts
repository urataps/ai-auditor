...
export function auditorAgent() {
  const systemMsg = `Your task is to analyze and assess smart contracts for auditing purposes by identifying the severity of the vulnerabilities, summarize them in a short title and description. Do not specify overflow/underflow vulnerabilities. The report should be generated in JSON format and should always follow the provided schema. Assume OpenZeppelin library is secure enough and that all the events are emitted there. The OpenZeppelin version is 5, so the _update function is used for transfers, mint and burn.`;
  ...
  return prompt.pipe(jsonAgent('gpt-4o', auditJsonSchema));
}
